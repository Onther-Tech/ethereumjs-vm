...
const runCall = require('./../runCall.js')
...
function makeCall (runState, callOptions, localOpts, cb) {
  ...
  function parseCallResults (err, results) {
    if (err) return cb(err)
    // @dev: store callee state here
    // TODO: move on application layer instead of VM itself
    runState.calleeProof = results.vm.runState.initStorageProof;
    runState.calleeSteps = results.vm.runState.steps;
    runState.calleeCode = results.vm.runState.rawCode;
    runState.calleeCallData = results.vm.runState.rawCallData;
    runState.calleeTstorage = results.vm.runState.tStorage;
