...
var runCall = require('./../runCall.js');
...
module.exports = {
  ...
  LOG: function LOG(memOffset, memLength) {
    ...
    subGas(runState, new BN(runState._common.param('gasPrices', 'logTopic')).imuln(numOfTopics).iadd(memLength.muln(runState._common.param('gasPrices', 'logData')))); // add address
  ...
  CREATE2: function CREATE2(value, offset, length, salt, runState, done) {
    ...
    var localOpts = {
      ...
      inLength: length,
      outOffset: new BN(0),
      outLength: new BN(0) // Deduct gas costs for hashing
...
function makeCall(runState, callOptions, localOpts, cb) {
  ...
  callOptions.storageReader = runState.storageReader; // increment the runState.depth
  ...
  if (runState.depth >= runState._common.param('vm', 'stackLimit') || callOptions.delegatecall !== true && new BN(runState.contract.balance).lt(callOptions.value)) {
    ...
    runState.stateManager.putAccount(runState.address, runState.contract, function (err) {
      if (err) return cb(err);
      runCall.call(runState._vm, callOptions, parseCallResults);
    });
  }
  ...
  function parseCallResults(err, results) {
    ...
    if (!results.vm.exceptionError) {
      Object.assign(runState.selfdestruct, selfdestruct); // update stateRoot on current contract
