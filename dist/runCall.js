...
var runJitFunc = require('./runJit.js');

var runCodeFunc = require('./runCode.js');
...
module.exports = function (opts, cb) {
  ...
  txValue = new BN(txValue); // run and parse
  ...
  function setupNewContract(address, done) {
    stateManager.clearContractStorage(address, function (err) {
      ...
      async.series(self.emit === undefined ? [getAccount] : [newContractEvent, getAccount], done);
  ...
  function runCode(cb) {
    ...
    var runCodeOpts = {
      ...
      static: isStatic,
      storageReader: storageReader // run Code through vm

    };
    var codeRunner = isCompiled ? runJitFunc : runCodeFunc;
