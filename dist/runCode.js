...
module.exports = function (opts, cb) {
  var self = this;
  var runState;
  this.initRunState(opts).then(function (state) {
    runState = state;
    runVm();
  }).catch(function (err) {
    cb(err);
  ...
  function iterateVm(done) {
    ...
    async.series(self.emit === undefined ? [runOp] : [runStepHook, runOp], function (err) {
    ...
    function runOp(cb) {
      self.runNextStep(runState).then(function () {
        cb();
      }).catch(function (err) {
        cb(err);
      });
