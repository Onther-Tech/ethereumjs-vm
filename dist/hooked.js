...
function createHookedVm(opts, hooks) {
  ...
  function loadAccount(address, cb) {
    ...
    async.parallel({
      ...
      results._exists = results.nonce !== '0x0' || results.balance !== '0x0' || results._code !== '0x'; // console.log('fetch account results:', results)

      var account = new Account(results); // not used but needs to be anything but the default (ethUtil.KECCAK256_NULL)
...
function fromWeb3Provider(provider, blockNumber, opts) {
  ...
  function createRpcFunction(provider, method, blockNumber) {
    return function sendRpcRequest() {
      ...
      var cb = args.pop();
      args.push(blockNumber); // send rpc payload
...
FallbackStorageTrie.prototype.get = function (key, cb) {
  ...
  _super(key, function (err, value) {
    if (err) return cb(err);
    if (value) return cb(null, value); // if value not in tree, try network
