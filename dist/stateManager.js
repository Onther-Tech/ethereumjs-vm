...
proto.putAccount = function (address, account, cb) {
  ...
  self._touched.add(address.toString('hex')); // self._trie.put(addressHex, account.serialize(), cb)
...
proto.checkpoint = function (cb) {
  ...
  self._touchedStack.push(new Set(_toConsumableArray(self._touched)));
...
proto.revert = function (cb) {
  ...
  self._trie.revert(); // setup cache checkpointing
